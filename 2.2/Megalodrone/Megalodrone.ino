/*
MultiCopter by Alexandre Dubus
...
void annexCode() { // this code is excetuted at each loop and won't interfere with control loop if it lasts less than 650 microseconds
  ...
  if(f.ARMED) {
    ...
    if(currentMillis - previousMillis > 800) { // thr change interval = 1.3 sec
      previousMillis = currentMillis;
      if (setTHR < 1400 && thr_down == 0){
        setTHR+=50;                            // thr change rate 
        if (setTHR >= 1400) thr_down = 1;
      }
      else if (setTHR > 1100 && thr_down == 1) {
        setTHR-=35;
        if (setTHR <= 1150) f.ARMED=0;
      }
    }
    rcData[THROTTLE]=1350;//setTHR; //***********throttle set manual 1250->2.2
//  rcData[ROLL]=setROLL;
//  rcData[YAW]=setYAW;
