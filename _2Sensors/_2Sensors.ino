...
void loop() {
  for (uint8_t i = 0; i < SONAR_NUM; i++) { // Loop through all the sensors.
    if (millis() >= pingTimer[i]) {         // Is it this sensor's time to ping?
      ...
      if (i == 0 && currentSensor == SONAR_NUM - 1) {oneSensorCycle(); Serial.println(sonarPid(cm[0]));} // Sensor ping cycle complete, do something with the results.
...
uint16_t sonarPid(uint16_t sonarAlt) {
  ...
  if (ARMED) {
    ...
    if (land == 0) {
      ...
      PTermSonar = errorSonar * PSonar * 5 + 1000;
      ...
      setTHR--;
      //TODO: send to flight controller
      if (SonarAlt <= 10)
        ARMED = 0;
    }
  }
